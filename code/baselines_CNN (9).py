...
from keras.layers import Activation,concatenate, Dense, Dropout, Embedding, Flatten, RepeatVector, Input, merge, Convolution1D, Convolution2D, MaxPooling1D, GlobalMaxPooling1D, LSTM, Bidirectional
...
def baseline_perceptron(sequence_len):
    ...
    # model.compile(loss=w_binary_crossentropy, optimizer='adadelta', metrics=[accuracy_score, recall_score])
...
def baseline_CNN(sequences_length_for_training, embedding_dim, embedding_matrix, vocab_size):
    ...
    # model.compile(loss=w_binary_crossentropy, optimizer='adadelta', metrics=['accuracy'])
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i in range(total_documents):
            ...
            for (batch_X, batch_Y) in batch_gen_sentences_without_context(X, Y, batch_size, fixed_size=False):
                ...
                batch_Y = to_categorical(batch_Y,2) # Convert to output as 2 classes
...
def testing_on_data(type_of_data, X_test, Y_test, model, batch_size, summary_only=False, visualize=False):
    ...
    if data['wd_e']:
      ...
      print "WinDiff metric (Windiff_e) %f " %(np.mean(data['wd_e']))
      print "Pk metric %f " %(np.mean(data['pk']))
      print('___________________________________')
      f_wd.append(np.mean(data['wd_e']))
      f_pk.append(np.mean(data['pk']))
...
if __name__ == "__main__":
    ...
    # For which_model == 2
    SAMPLE_TYPE_wiki, X_wiki, Y_wiki, trained_sample_handler = get_input(sample_type=5, shuffle_documents=True, pad=False)
    ...
    #pdb.set_trace()
    train_model(X_wiki, Y_wiki, model,train_split=0.8, epochs=25, batch_size=64)
